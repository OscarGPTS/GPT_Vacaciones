...
<head>
    ...
    <title>Solicitud de Vacaciones Actualizada</title>
    <style>
        body {
...
<body>
    <div class="container">
        <div class="header">
            ...
            <h1>&#9998; Solicitud de Vacaciones Actualizada</h1>
        </div>
        ...
        <div class="content">
            ...
            @if($editedBySelf)
            <p>Has modificado tu solicitud de vacaciones. A continuación encontrarás el resumen con los cambios realizados.</p>
            @else
            <div class="notice-box">
                <strong>Aviso:</strong> Tu solicitud de vacaciones fue modificada por
                <strong>{{ $editedBy->first_name }} {{ $editedBy->last_name }}</strong>
                del área de Recursos Humanos.
                Si tienes alguna duda, comunícate directamente con el departamento de RH.
            </div>
            @endif

            <div class="info-box">
                <h3>Detalle de la Solicitud Actualizada</h3>
                <div class="info-row">
                ...
                @if($request->opcion)
                    @php
                        $periodNum = explode('|', $request->opcion)[0] ?? null;
                    @endphp
                    @if($periodNum)
                    <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    {{ $editedBy->first_name }} {{ $editedBy->last_name }}
                </div>
                @endif
            </div>
            ...
            <div class="days-list">
                <div class="days-list-title">Días de ausencia registrados:</div>
                @foreach($request->requestDays->sortBy('start') as $day)
                <div class="day-item">
                    {{ \Carbon\Carbon::parse($day->start)->locale('es')->isoFormat('dddd D [de] MMMM [de] YYYY') }}
                </div>
                @endforeach
            </div>
            @endif
            ...
            <p style="text-align: center;">
                <a href="{{ route('vacaciones.index') }}" class="button">
                    VER MIS SOLICITUDES
                </a>
            </p>

            <p style="color: #666666; font-size: 14px;">
                Tu solicitud sigue pendiente de aprobación. Recibirás una notificación cuando sea revisada por tu jefe directo.
            </p>
        </div>

        <div class="footer">
            <p style="margin: 0 0 10px 0;">Este es un correo automático generado por el sistema.</p>
            <p style="margin: 0;"><span class="footer-accent">GPT Services</span> | Sistema de Recursos Humanos</p>
