    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <x-button green  label="PDF"
                        href="{{ route('rrhh.requisiciones.curso.pdf', $requisicion->id) }}" />
                    @if ($requisicion->status()->canBe('Cerrada'))
                        <x-button red
                            label="Cerrar" href="{{ route('rrhh.requisiciones.curso.cerrar', $requisicion->id) }}" />
                    @endif
                </div>
            </div>
        </div>
            ...
            <div class="col-md-12">
                ...
                <ul class="list-group">
                    @foreach ($participantes as $participante)
                        <li class="list-group-item">{{ $participante->nombre() }} - {{ $participante->job->name }}
                        </li>
                    @endforeach
                </ul>
            </div>
