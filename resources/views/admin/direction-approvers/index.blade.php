            ...
            <div class="card shadow-sm">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <h3 class="card-title mb-0 text-black">
                                <i class="fas fa-user-shield me-2"></i> Gestión de Aprobadores de Dirección
                            </h3>
                            <small class="text-black d-block mt-1">
                                Asigna usuarios que puedan aprobar solicitudes de vacaciones de departamentos específicos
                            </small>
                        </div>
                        <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addApproverModal">
                            <i class="fas fa-plus me-1"></i> Asignar Nuevo Aprobador
                        </button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info mb-4">
                        ...
                        <p class="mb-0 mt-2">
                            Permite asignar usuarios que puedan aprobar solicitudes de vacaciones en la etapa de "Dirección" 
                            sin necesidad de tener el puesto de Dirección. Cada usuario puede ser asignado a uno o más departamentos.
                        </p>
                    </div>
                    ...
                    @else
                        <div class="text-center py-5">
                            ...
                            <h5 class="text-muted">No hay aprobadores de dirección asignados</h5>
                            <p class="text-muted">Haz clic en "Asignar Nuevo Aprobador" para comenzar</p>
                        </div>
                    @endif
                </div>
            </div>
...
<!-- Modal para Asignar Aprobador -->
<div class="modal fade" id="addApproverModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content bg-white">
            <form action="{{ route('admin.direction-approvers.store') }}" method="POST">
                @csrf
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">
                        <i class="fas fa-user-plus me-2"></i>
                        Asignar Nuevo Aprobador de Dirección
                    </h5>
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle me-2"></i>
                        <strong>Asignación por departamento:</strong> Aprobará todas las solicitudes del departamento.<br>
                        <strong>Asignación por usuario específico:</strong> Solo aprobará solicitudes del/los usuario(s) seleccionado(s) (prioridad sobre departamento).
                    </div>

                    <div class="mb-3">
                        <label for="user_id_dir" class="form-label">
                            <i class="fas fa-user me-1"></i> Usuario Aprobador <span class="text-danger">*</span>
                        </label>
                        <select name="user_id" id="user_id_dir" class="form-select" required>
                        ...
                        <small class="text-muted">Usuario que aprobará en nivel de Dirección</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="assignment_type" id="type_departamento_dir" value="departamento" checked>
                            <label class="btn btn-outline-primary" for="type_departamento_dir">
                                <i class="fas fa-building"></i> Por Departamento
                            </label>

                            <input type="radio" class="btn-check" name="assignment_type" id="type_usuario_dir" value="usuario">
                            <label class="btn btn-outline-primary" for="type_usuario_dir">
                    ...
                    <div id="departamentosSectionDir" class="mb-3">
                        <label class="form-label">
                            ...
                            @foreach($departamentos as $dept)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="departamentos[]" 
                                           value="{{ $dept->id }}" id="dept_dir{{ $dept->id }}">
                                    <label class="form-check-label" for="dept_dir{{ $dept->id }}">
                                        {{ $dept->name }}
                                    </label>
                                </div>
                            @endforeach
                    ...
                    <div id="usuariosSectionDir" class="mb-3" style="display: none;">
                        <label class="form-label">
                            Usuarios Específicos <span class="text-danger">*</span>
                        </label>
                        <small class="text-muted d-block mb-2">Selecciona uno o más usuarios que este aprobador gestionará</small>
                        <div class="border rounded p-3" style="max-height: 300px; overflow-y: auto;">
                            @foreach($users as $user)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="usuarios_especificos[]" 
                                           value="{{ $user->id }}" id="userApproveDir{{ $user->id }}">
                                    <label class="form-check-label" for="userApproveDir{{ $user->id }}">
                                        {{ $user->first_name }} {{ $user->last_name }}
                                        @if($user->job)
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-1"></i> Asignar Aprobador
                    </button>
                </div>
            </form>
        </div>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-bold">
                            <i class="fas fa-user me-1"></i> Aprobador de Dirección <span class="text-danger">*</span>
                        </label>
...
document.getElementById('type_departamento_dir').addEventListener('change', function() {
    document.getElementById('departamentosSectionDir').style.display = 'block';
    document.getElementById('usuariosSectionDir').style.display = 'none';
    document.querySelectorAll('input[name="usuarios_especificos[]"]').forEach(el => el.checked = false);
});

document.getElementById('type_usuario_dir').addEventListener('change', function() {
    document.getElementById('departamentosSectionDir').style.display = 'none';
    document.getElementById('usuariosSectionDir').style.display = 'block';
...
function toggleApprover(id, employeeName) {
    if (confirm(`¿Cambiar el estado de la asignación de ${employeeName}?`)) {
        ...
        form.action = `/admin/direction-approvers/${id}/toggle`;
        form.submit();
    }
}
...
function deleteApprover(id, employeeName) {
    if (confirm(`¿Está seguro de eliminar la asignación de ${employeeName}?\n\nEsta acción no se puede deshacer.`)) {
        ...
        form.action = `/admin/direction-approvers/${id}`;
        form.submit();
    }
    return false;
}
...
function deleteAllForBoss(bossId, bossName) {
    if (confirm(`¿Eliminar TODAS las asignaciones de ${bossName}?\n\nSe eliminarán todos los empleados asignados. Esta acción no se puede deshacer.`)) {
        ...
        form.action = `/admin/direction-approvers/${bossId}/all`;
        form.submit();
    }
}
...
function openEditModal(bossId) {
    ...
    form.action = `/admin/direction-approvers/${bossId}`;
