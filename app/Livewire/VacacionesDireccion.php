...
class VacacionesDireccion extends Component
{
    use WithPagination;
    ...
    public function approveRequest()
    {
        if ($this->selectedRequest) {
            try {
                ...
                session()->flash('success', "Solicitud aprobada por Dirección. Ahora pasará a Recursos Humanos para aprobación final.");
                
                // NO enviar notificación al empleado cuando Dirección aprueba
                // El empleado ya fue notificado cuando su jefe directo aprobó
                // Solo será notificado cuando RH haga la aprobación final

                // Enviar notificación a RH
                    ...
                    if ($rhUser && $rhUser->email) {
                        try {
                            ...
                            Mail::to($rhUser->email)
                                ->send(new \App\Mail\VacationRequestPendingRH($this->selectedRequest));
                            Log::info('Correo enviado a RH desde Dirección', [
                                'request_id' => $this->selectedRequest->id,
                                'rh_email' => $rhUser->email
                            ]);
                        } catch (\Exception $e) {
    ...
    public function rejectRequest()
    {
        if ($this->selectedRequest) {
            try {
                ...
                session()->flash('success', 'Solicitud rechazada por Dirección. Los días han sido liberados.');
                ...
                if ($this->selectedRequest->user && $this->selectedRequest->user->email) {
                    try {
                        ...
                        Mail::to($this->selectedRequest->user->email)
                            ->send(new \App\Mail\VacationRequestRejectedByDirection($this->selectedRequest));
                        Log::info('Correo de rechazo de Dirección enviado al empleado', [
                            'request_id' => $this->selectedRequest->id,
    ...
    public function getProcessedRequestsProperty()
    {
        if ($this->statusFilter === 'processed') {
            return $this->getFilteredRequests()
                ->where('direct_manager_status', 'Aprobada')
                ->whereIn('direction_approbation_status', ['Aprobada', 'Rechazada'])
                ->orderBy($this->sortField, $this->sortDirection)
                ->paginate($this->perPage);
        }
        return collect([]);
    ...
    private function getFilteredRequests()
    {
        ...
        // FILTRAR POR APROBADOR DE DIRECCIÓN ASIGNADO
        // LÓGICA CORRECTA: Solo mostrar solicitudes donde el usuario autenticado 
        // sea EXACTAMENTE el direction_approbation_id asignado en la solicitud.
        // Esto respeta la configuración de direction_approvers que se asigna 
        // automáticamente cuando el jefe directo aprueba.
        
        $query->where('direction_approbation_id', auth()->id());
